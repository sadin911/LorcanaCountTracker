/**
 * Fast Web Speech API Text-to-Speech (TTS) Engine for Lorcana Cards.
 * Optimized for snappy feedback during hands-free card sorting and voice collection.
 */

export interface SpeakOptions {
  lang?: 'th-TH' | 'en-US';
  rate?: number; // 1.0 - 2.0 (Fast rate default 1.45 for rapid card counting)
  pitch?: number;
  volume?: number;
}

const DEFAULT_RATE = 1.45;
...
/**
 * Normalizes collector number for speech (e.g. '025/204' -> '25', '004' -> '4')
 */
 ...
 * Includes card number and Lorcana card name so user can hands-free verify.
 * Automatically adapts to English if the device lacks a Thai TTS voice.
 * e.g. "เบอร์ 25 Elsa ฟอยล์ 2 ใบ", "Number 25, Elsa Foil, 2 cards"
 */
export function formatCardSpokenText(
  ...
  if (useEnglish) {
    ...
    const finishSuffix = finish === 'foil' ? ' Foil' : '';
    const qtyText = quantity > 1 ? `, ${quantity} cards` : '';
    return `${numPrefix}${cleanName}${finishSuffix}${qtyText}`.trim();
  }

  // Thai TTS phrasing
  const finishText = finish === 'foil' ? ' ฟอยล์' : '';
  const numPrefix = cleanNum ? `เบอร์ ${cleanNum} ` : '';
  const qtyText = quantity > 1 ? ` ${quantity} ใบ` : '';

  return `${numPrefix}${cleanName}${finishText}${qtyText}`.trim();
}
