...
export function calculateDeckStats(
  ...
  for (const [cardId, entry] of Object.entries(deck.cards)) {
    ...
    if (nameCounts[fullName] > 4) {
      ruleViolations.push(`"${fullName}" exceeds 4 copies limit (${nameCounts[fullName]} currently in deck)`);
    }
  ...
  if (activeInks.length > 2) {
    ruleViolations.push(`Deck has more than 2 Ink colors (${activeInks.join(', ')}). Maximum allowed is 2 colors.`);
  }

  // Deck size legality (Lorcana standard constructed deck is minimum 60 cards)
  if (totalCards < 60) {
    ruleViolations.push(`Deck requires at least 60 cards (currently ${totalCards} cards, ${60 - totalCards} remaining).`);
  }
...
export function generateShoppingListText(deckName: string, report: DeckMissingReport): string {
  if (report.missingItems.length === 0) {
    return `🎉 Deck "${deckName}" is 100% complete! No missing cards needed.`;
  }

  const lines = [
    `📋 [Missing Cards Shopping List] Deck: ${deckName}`,
    `📊 Missing: ${report.totalCardsMissing} cards (Owned: ${report.totalCardsOwned}/${report.totalCardsNeeded} - ${report.completionPercentage}%)`,
    `----------------------------------------`,
  ];
  ...
  for (const [ink, items] of Object.entries(inkGroups)) {
    ...
    items.forEach((item) => {
      ...
      lines.push(
        ` • ${item.fullName} ${costBadge} ${setTag} - Missing ${item.missingCount} (Need ${item.countNeeded}, Owned ${item.countOwned})`
      );
    });
  }

  lines.push(`\n----------------------------------------`);
  lines.push(`✨ Generated by Disney Lorcana Count Tracker Deck Builder`);
  return lines.join('\n');
}
