...
const STORAGE_KEY_TAB = 'lorcana_import_active_tab';
...
export function CollectionTextImportModal({ onClose, initialTab }: Props) {
  ...
  const importBatchCards = useCollectionStore((s) => s.importBatchCards);
  ...
  // Live parse preview as the user types or pastes in text tab
  ...
  const handleVoiceImport = (
    cards: Array<{ cardId: string; quantity: number; finish: FinishKey }>
  ) => {
    setStatus(null);
    if (cards.length === 0) {
      setStatus({ ok: false, message: 'No cards selected to import.' });
      return;
    }

    const res = importBatchCards(cards, {
      mode,
  ...
  return createPortal(
    ...
    >
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#c8b07b]/20 bg-[#1b2038]/80 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">
                Add cards via Voice Recognition or Text list
              </p>
            </div>
          </div>
          <button
        ...
        {/* Tab Switcher */}
        <div className="flex items-center px-4 pt-2.5 gap-2 border-b border-[#c8b07b]/20 bg-[#131627]/60 shrink-0">
          <button
            type="button"
            data-testid="voice-tab-button"
            onClick={() => setActiveTab('voice')}
            className={`px-4 py-2 rounded-t-xl font-extrabold text-xs flex items-center gap-1.5 transition-all border-b-2 ${
              activeTab === 'voice'
                ? 'border-[#dfc792] text-[#dfc792] bg-[#1b2038] shadow-sm'
                : 'border-transparent text-slate-400 hover:text-slate-200'
            }`}
          >
            <span>🎙️</span>
            <span>Voice Studio</span>
            <span className="px-1.5 py-0.2 rounded bg-[#c8b07b]/20 text-[#dfc792] text-[9px] font-black uppercase">
              Live
            </span>
          </button>
        ...
        <div className="p-4 sm:p-5 overflow-y-auto scrollbar-thin space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 p-3 rounded-2xl bg-[#1b2038] border border-[#c8b07b]/20">
            {/* Target Binder */}
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full px-2.5 py-1.5 rounded-xl bg-[#131627] border border-[#c8b07b]/30 text-xs text-slate-200 focus:outline-none focus:border-[#c8b07b]"
              >
                {binderList.map((b) => (
